...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                                                                                                                                                                            ...
                                                                                                                                                                                                            this.zhuangtaiOptions = "待处理,已查阅,预约面试,通过,不通过".split(',')
                                                                },
                                                                                                        // 多级联动参数
                                                                                                                                    info(id) {
      this.$http({
        url: `yingpinzhuangkuang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `yingpinzhuangkuang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yingpinzhuangkuangCrossAddOrUpdateFlag = false;
                  this.parent.search();
